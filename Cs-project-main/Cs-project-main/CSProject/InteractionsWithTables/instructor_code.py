...
def __check_if_class_exists(name, section):
    data = common_functions.__read_file_csv(subject_file)
    for subject in data:
        if subject and subject[1] == name and subject[4] == section:
            return True
...
def __check_if_question_exists(question, subject):
    ...
    for q in data:
        if q and q[1] == question and q[7] == subject:
            return True
    return False
...
def __find_question_by_id(question_id):
    ...
    for question in questions:
        if question and question[0] == question_id:
            return question
...
def edit_subject(subject_id, new_name=None, new_section=None, new_passing_grade=None, new_code=None): #Done
    ...
    new_subject = None
    for subject in data:
        if subject and subject[0] == str(subject_id):
            new_subject = subject
            data.remove(subject)
            if new_name:
